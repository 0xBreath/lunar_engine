...
impl std::fmt::Display for RetrogradeError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            RetrogradeError::TimeError(e) => write!(f, "TimeError: {}", e),
        }
    }
}

pub type RetrogradeResult<T> = Result<T, RetrogradeError>;
...
impl Retrograde {
            ...
            for index in 0..daily_angles.len() - 1 {
                ...
                // end of retrograde
                    ...
                    if retro_start_date.is_some() && retro_start_angle.is_some() {
                        let start_date =
                            retro_start_date.expect("failed to unwrap retrograde start date");
                        let start_angle =
                            retro_start_angle.expect("failed to unwrap retrograde start angle");
    ...
    pub fn confluent_retrograde(
        ...
        for index in 0..period {
            ...
            if events_on_date.len() > 1 {
                ...
                println!("{}\t{}", date.to_string(), events_on_date.len());
            }
        }
        Ok(confluent_retrogrades)
    }
    ...
    pub fn print(&self) {
        for retrograde in self.retrogrades.iter() {
            println!(
                "{}\t{}\t{}",
                retrograde.planet.to_str(),
                retrograde.start_date.to_string(),
                retrograde.end_date.to_string()
            );
        }
    }
}
