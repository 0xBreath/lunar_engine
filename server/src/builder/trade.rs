...
impl BinanceTrade {
  pub fn new(symbol: String, side: Side, order_type: OrderType, quantity: f64, price: Option<f64>, stop_loss: Option<f64>, trailing_stop: Option<f64>) -> Self {
    Self {
      symbol,
      side,
      order_type,
  ...
  fn get_timestamp(&self) -> Result<u64> {
    ...
    let since_epoch = system_time.duration_since(UNIX_EPOCH).expect("System time is before UNIX EPOCH");
  ...
  //     btree.insert("trailingDelta".to_string(), trailing_stop.to_string());
  //   }
  ...
  fn build(&self) -> Vec<(String, String)> {
    ...
    data.push(("symbol".to_string(), self.symbol.clone()));
    data.push(("side".to_string(), self.side.fmt_binance().to_string()));
    data.push(("type".to_string(), self.order_type.fmt_binance().to_string()));
    if self.order_type == OrderType::StopLossLimit
      || self.order_type == OrderType::Limit
      || self.order_type == OrderType::TakeProfitLimit
    {
      data.push(("timeInForce".to_string(), "GTC".to_string()));
    }
    data.push(("quantity".to_string(), self.quantity.to_string()));
    if let Some(price) = self.price {
      data.push(("price".to_string(), price.to_string()));
    }
    if let Some(trailing_stop) = self.trailing_stop {
      data.push(("trailingDelta".to_string(), trailing_stop.to_string()));
    }
    if let Some(stop_loss) = self.stop_loss {
      data.push(("stopPrice".to_string(), stop_loss.to_string()));
    }
    let timestamp = self.get_timestamp().expect("Failed to get timestamp");
    data.push(("timestamp".to_string(), timestamp.to_string()));
    data
  }
  ...
  pub fn bips_trailing_stop(trailing_stop_pct: f64) -> f64 { trailing_stop_pct * 100.0 }
