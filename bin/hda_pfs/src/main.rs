...
async fn main() -> PFSResult<()> {
    ...
    let spx_hda_pfs_backtest_file =
        path_to_dir.clone() + "/data/SPX/output/SPX_HDA_PFS_confluent_direction_backtest.csv";
    ...
    spx_ticker_data
        .build_series("SPX", Interval::Daily, &PathBuf::from(spx_daily))
        .await
        .map_err(PFSError::TickerDataError)?;
    ...
    write_hda_pfs_backtest_csv(backtests, &spx_hda_pfs_backtest_file)?;
    Ok(())
}
...
fn write_hda_pfs_backtest_csv(backtests: Vec<Backtest>, out_file: &str) -> PFSResult<()> {
    if backtests.is_empty() {
        return Err(PFSError::BacktestEmpty);
    }
    let mut file = File::create(out_file).map_err(PFSError::CustomError)?;
