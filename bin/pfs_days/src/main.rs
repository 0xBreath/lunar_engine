...
async fn main() {
    ...
    let pfs_confluent_cycles_raw = env::var("PFS_CONFLUENT_CYCLES")
      .expect("PFS_CONFLUENT_CYCLES not set");
    let mut pfs_confluent_cycles: Vec<&str> = pfs_confluent_cycles_raw.split(',').collect();
    pfs_confluent_cycles = pfs_confluent_cycles.iter()
      .map(|&x| x.trim())
      .collect();
    // map to u32
...
async fn btcusd_individual_pfs_backtest(
    ...
    for cycle in cycles {
        pfs_cycles.push(PlotPFS::pfs_days(pfs.start_date, pfs.end_date, ticker_data, cycle))
    };
...
async fn btcusd_pfs_confluent_reversal(
    ...
    for cycle in pfs_confluent_cycles.iter() {
        pfs_cycles.push((*cycle, PlotPFS::pfs_days(pfs.start_date, pfs.end_date, ticker_data, *cycle)));
    }
    let timeframe = PFSTimeframe::Day;
    let backtest_corr = pfs.confluent_pfs_reversal(ticker_data, pfs_confluent_cycles, timeframe,&pfs_confluence_file);
...
fn write_backtest_csv(backtests: Vec<(Backtest, Vec<u32>)>, out_file: &str) -> Result<(), Box<dyn Error>> {
    ...
    for backtest in backtests.iter() {
        ...
        let cycles = backtest.1.iter().map(|cycle| cycle.to_string()).collect::<Vec<String>>().join(",");
        let start_date = backtest.0.start_date.expect("No start date found").to_string_daily();
        let end_date = backtest.0.end_date.expect("No end date found").to_string_daily();
