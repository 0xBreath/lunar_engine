...
impl TickerData {
  ...
  pub fn candle_is_high(&self, candle: &Candle, left_bars: usize, right_bars: usize, error_margin: usize) -> bool {
    let local_highs = self.pivot_highs(left_bars, right_bars);
  ...
  pub fn candle_is_low(&self, candle: &Candle, left_bars: usize, right_bars: usize, error_margin: usize) -> bool {
    let local_lows = self.pivot_lows(left_bars, right_bars);
  ...
  pub fn candle_is_reversal(&self, candle: &Candle, left_bars: usize, right_bars: usize, error_margin: usize) -> bool {
  ...
  pub fn find_sushi_roll_reversals(&self, period: usize) -> Vec<Reversal> {
    ...
    for (index, index_candle) in self.candles.iter().enumerate() {
      ...
      debug!("Date: {}\tMean High: {}\tMean Low: {}", index_candle.date.to_string(), mean_high, mean_low);
