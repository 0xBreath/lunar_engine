...
impl MarketStructure {
  ...
  pub fn new(ticker_data: &TickerData, candle_range: usize) -> Self {
    ...
    println!("First Candle: {:?}", ticker_data.candles[0].date.to_string());
    println!("Last Candle: {:?}", ticker_data.candles[ticker_data.candles.len() - 1].date.to_string());
    println!("First Reversal: {:?}", reversals[0].candle.date.to_string());
    println!("Last Reversal: {:?}", reversals[reversals.len() - 1].candle.date.to_string());
